...
elab (name := notation3) doc:(docComment)? attrs?:(Parser.Term.attributes)? attrKind:Term.attrKind
    "notation3bis" prec?:(precedence)? name?:(namedName)? prio?:(namedPrio)?
  ...
  if getPrettyPrintOpt pp? then
    matchers := matchers.push <| Mathlib.Notation3bis.mkExprMatcher val boundNames
...
macro_rules
  | `($[$doc]? $(attr)? scoped[$ns] notation3bis $(prec)? $(n)? $(prio)? $(pp)? $items* => $t) =>
    `(with_weak_namespace $(mkIdentFrom ns <| rootNamespace ++ ns.getId)
      $[$doc]? $(attr)? scoped notation3bis $(prec)? $(n)? $(prio)? $(pp)? $items* => $t)
