...
declare_trace parens

meta def op_symb : expr → option string
| `(%%x * %%y) := pure " * "
| `(%%x / %%y) := pure " / "
| `(%%x + %%y) := pure " + "
| `(%%x - %%y) := pure " - "
...
meta def sf.add_parens : sf → tactic sf
| s@(sf.tag_expr addr e a) := do
  bool.tt ← tactic.get_bool_option `trace.parens ff | pure s,
  some symbole ← pure (op_symb e) | pure s,
  let a1 := [app_fn, app_arg],
  (e1, s1) ← sf.follow a1 a,
  let a2 := [app_arg],
  (e2, s2) ← sf.follow a2 a,
  pure (sf.tag_expr addr e $
    "(" ++ (sf.tag_expr (addr ++ a1) e1 s1) ++ symbole ++ (sf.tag_expr (addr ++ a2) e2 s2) ++ ")")
| x := pure x
...
meta def tactic.interactive.parentheses  := tactic.set_bool_option `trace.parens tt
